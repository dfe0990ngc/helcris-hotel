...
import { guestUpdateUserProfile, guestUpdateUserImage, guestUpdateUserPassword } from '../../api/api.js';
...
const GuestProfile: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try{
      const { data } = await guestUpdateUserProfile(user?.id, formData);
  ...
  const handleUpdateImageUrl = async (e) => {
    ...
    try{
      const { data } = await guestUpdateUserImage(user?.id, {profile_url: user?.profile_url});
  ...
  const handlePasswordChange = async () => {
    ...
    try{
      const { data } = await guestUpdateUserPassword(user?.id, {
